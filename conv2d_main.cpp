...
int main(int argc, char *argv[])
{
	...
	int i, reps=1, size, iterations=1,mhz, k_size, verify =0;
    char *stat_file = NULL;
	...
	if(verify == 1)
            sprintf(header,"size,k_size,function,IC,Cycles,CPI,CT,ET,L1_dcache_miss_rate,L1_dcache_misses,L1_dcache_accesses,correctness");
	else
	   sprintf(header,"size,k_size,function,IC,Cycles,CPI,CT,ET,L1_dcache_miss_rate,L1_dcache_misses,L1_dcache_accesses");
		...
		for(auto & size:sizes) {
			for(auto & k_size: k_sizes ) {
				for(auto & function : functions) {
					...
					sprintf(preamble, "%lu,%d,%s,",size,k_size,function.c_str());
					perfstats_init();
